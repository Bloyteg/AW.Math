    ...
    public class Triangulator
    {
        ...
        public Triangulator(IList<Vector3> vertices, IEnumerable<int> indices)
        {
            var countour = indices.Select(index =>
                ...
                {
                    Data = index,
                    Position = new Vec3 { X = (float)vertices[index].X, Y = (float)vertices[index].Y, Z = (float)vertices[index].Z }
                }).ToArray();

            _tessellator.AddContour(countour, ContourOrientation.Original);
        }
