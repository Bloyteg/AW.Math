...
module Vector = 
    let inline pack x y z = 
        (^R : (static member Make : ^S * ^S * ^S -> ^R) (x, y, z))

    let inline unpack vector =
        let x = (^T : (member X : ^S) vector)
        let y = (^T : (member Y : ^S) vector)
        let z = (^T : (member Z : ^S) vector)
        (x, y, z)
    ...
    let inline normalized vector : ^R = 
        let (x, y, z) = unpack vector
